...
function lsm(pp:ProjectProcess)
    ...
    value_func = pp.CashProcess.terminal_multiplier * (terminal_cash_flow .> 0) .* terminal_cash_flow
    ...
    # Determine size of the basis function matrix for value function approximation
    # number_basis_cols = 
    ...
    @inbounds for period in (periods-1):-1:1
        ...
        # Initalize basis matrix for regression
        basis_matrix = pp.SimOptions.basis.(cash_matrix[:,period],cost_matrix[:,period])
        basis_matrix = reduce(hcat,basis_matrix)' # vector of vectors to matrix
